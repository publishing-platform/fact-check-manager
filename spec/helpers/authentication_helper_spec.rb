...
RSpec.describe AuthenticationHelper, type: :helper do
  ...
  let(:request_record) { build(:request) }
  ...
  describe "#generate_compare_preview_link" do
    it "returns the correct link with the token" do
      ...
      expect(link).to eq("#{PublishingPlatformLocation.external_url_for('fact-check-manager')}/requests/publisher/123/compare?token=#{helper.compare_preview_jwt_token(request_record)}")
    end
  end
  ...
  describe "#generate_compare_link" do
    it "returns the correct link" do
      ...
      expect(link).to eq("#{PublishingPlatformLocation.external_url_for('fact-check-manager')}/requests/publisher/123/compare")
    end
  end
  ...
  describe "#draft_origin_preview_url" do
    it "returns a draft origin URL with a JWT token" do
      ...
      expect(url).to eq("#{PublishingPlatformLocation.external_url_for('draft-origin')}/#{request_record.draft_slug}?token=#{token}")
    end
